 ...
 */
export function updateCategoriesToIncludeBitField(
    bitField: number,
    categoriesToInclude: { games?: boolean; software?: boolean; music?: boolean; hidden?: boolean }
) {
    let onMask = 0
    let offMask = 0
...
export function getIncludedCategoriesFromBitField(bitField: number) {
    ...
    for (const key of Object.keys(IncludeCategories).filter(key => isNaN(Number(key)))) {
        includes[key as keyof typeof IncludeCategories] =
            (IncludeCategories[key as keyof typeof IncludeCategories] & bitField) !== 0
    }
    return includes
}
 ...
 */
export function addPatch(route: string, patch: RoutePatch) {
    ...
    const existingPatches = [...(DeckyPluginLoader.routerHook.routerState._routePatches.get(route) ?? [])].filter(
        existingPatch => patch.toString() === existingPatch.toString()
    )
    existingPatches.forEach(patch => routerHook.removePatch(route, patch as RoutePatch))
